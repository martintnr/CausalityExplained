...
Causality_Explained <- function(CutOff = 5e-08, DataPath, Outcomes, NBCores = 1, BonferroniCorrection = F, MinNbrIVs = 1){
  ...
  resAllOutcomes <- mclapply(Outcomes, function(outcome){ # run the pipeline on all outcomes
    ...
    if ("variant_id" %in% names(Outcome)) {
      names(Outcome)[names(Outcome) == "variant_id"] <- "SNPid"}
    
    if ("standard_error" %in% names(Outcome)) {
      names(Outcome)[names(Outcome) == "standard_error"] <- "Sd"}
    
    if ("beta" %in% names(Outcome)) {
    ...
    Outcome <- Outcome[match(row.names(UKBBData_AllSNPs), Outcome$SNPid), c("Beta", "Sd")]
